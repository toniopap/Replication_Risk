...
## Prepare data for model estimation with ML 
...
ml_data <- genDesign(respondents = nrow(allcountries)) %>% 
  ...
  left_join(allcountries %>%  select(starts_with("series"), Country, ID, IDfarm, c("Age", "NbChildren", "Trust" , "FarmSize" , "LandOwned" , "IndivOwner", "pw" , "DateEnquete" , "Difficult" , "ChoiceRandom") ) , by="ID"  ) %>% 
...
ml_data_short    <- ml_data %>% 
  filter(as.character(Country) != "BJR2014" , ( is.na(Difficult) | Difficult < 5  ), (is.na(ChoiceRandom) | ChoiceRandom<5) ) %>% 
...
if(c!="All Countries")  ml_data<- ml_data_backup[ml_data_backup$Country==c,] 
...
## save models
save(list=(ls(pattern = "mlmodels_")), file = "storedResults/mlmodels_rob3.RData")
