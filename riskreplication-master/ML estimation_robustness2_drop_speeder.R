...
ml_data <- genDesign(respondents = nrow(allcountries)) %>% 
  ...
  left_join(allcountries %>%  select(starts_with("series"), Country, ID, IDfarm, c("Age", "NbChildren", "Trust" , "FarmSize" , "LandOwned" , "IndivOwner", "pw" , "DateEnquete", "Duration") ) , by="ID"  ) %>% 
...
ml_data_short    <- ml_data %>% 
  filter(!as.character(Country) %in% c("Austria", "Spain" , "Italy", "BJR2014"), Duration > hms::as_hms("00:06:00")) %>% 
...
## save models
save(list=(ls(pattern = "mlmodels_")), file = "storedResults/mlmodels_rob2.RData")
